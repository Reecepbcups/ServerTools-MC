...
import me.reecepbcups.core.*;
import me.reecepbcups.disabled.*;
import me.reecepbcups.events.*;
import me.reecepbcups.moderation.*;
import me.reecepbcups.runnables.*;
...
public class Main extends JavaPlugin implements Listener {	
	...
	public void loadLocalServerVariableKeys() {
		for(final String key : mainConfig.getConfigurationSection("PluginVariables").getKeys(false)){
	...
	public static String replaceVariable(String line) {
		...
		if (Arrays.stream(SERVER_VARIABLE_KEYS.toArray(new String[SERVER_VARIABLE_KEYS.size()]))
				.anyMatch(line::contains)) {

			//loop through keys, and if the line contains %TYPE%, replace it
			for (final String key : SERVER_VARIABLE_KEYS) {
